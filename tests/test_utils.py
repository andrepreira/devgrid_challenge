...
async def test_process_data():
    ...
    parsed_weather_data = [{'city_id': 1, 'weather': 'sunny'}, {'city_id': 2, 'weather': 'rainy'}, {'city_id': 3, 'weather': 'cloudy'}]

    mock_fetch_all_weather_data.return_value = weather_data
    mock_parse_weather_data.side_effect = parsed_weather_data

    with patch('app.utils.fetch_all_weather_data', mock_fetch_all_weather_data), \
         patch('app.utils.parse_weather_data', mock_parse_weather_data), \
         patch('app.crud.create_weather_data', mock_create_weather_data), \
         patch('app.crud.update_request_log', mock_update_request_log):

        result = await process_data(db=MagicMock(), city_ids=city_ids, request_log=request_log)
