...
from app.crud import create_weather_data, create_request_log, get_request_log_by_username, update_request_log, get_weather_data_by_city_id
from app.models import RequestLog, WeatherData
from app.schemas import RequestLogCreate, WeatherDataCreate, Weather
...
def test_get_request_log_by_username(mock_db_session):
    username = "test_user"
    mock_db_session.query.return_value.filter.return_value.first.return_value = RequestLog(username=username)
...
def test_get_weather_data_by_city_id(mock_db_session):
    city_id = 1
    mock_db_session.query.return_value.filter.return_value.first.return_value = WeatherData(city_id=city_id)
